...
const String geminiApiKey = "AIzaSyAGAaOWCis3WJM3bQ46DYIRGU4WK1mtjkw"; // ⬅️ ضع المفتاح هنا
  ...
  Future<void> _processImage(XFile image) async {
    ...
    if (response.text != null && response.text!.isNotEmpty) {
      try {
        ...
        final cleanJson = response.text!.trim().replaceAll('```json', '').replaceAll('```', '');
        final Map<String, dynamic> jsonResult = jsonDecode(cleanJson);
        
        final newInvoice = SimpleInvoice.fromJson({...jsonResult, 'id': DateTime.now().millisecondsSinceEpoch.toString()}); 
        
        // حفظ الفاتورة الجديدة محلياً
        final updatedInvoices = List<SimpleInvoice>.from(_invoices)..add(newInvoice);
        await LocalStorageService.saveInvoices(updatedInvoices);

        _showSuccessDialog(newInvoice);
        _loadInvoices(); // إعادة تحميل القائمة
      } catch (e) {
        _showErrorDialog('خطأ في قراءة البيانات',
